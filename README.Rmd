...
p <- g %>%
  ...
  geom_abline(intercept = 0, slope = slope_mean, linetype = "dashed", size = rel(0.7),
    color = "#bb8888cc") +
  ...
  theme(plot.background = element_rect(fill = "#f3f3f3", color = "transparent"),
    ...
    plot.subtitle = element_text(family = font,size = rel(1.274),
  ...
  labs(x = "Days Since First Case",
    y = "Reported Cases (logarithm scale)",
    subtitle = "Individual and average rates of reported cases by state according to covidtracking.com",
  ...
  ggsave("man/figures/README-state-trajectories.png",
    width = 8, height = 8, units = "in", dpi = 312)

betas <- unlist(dapr::lap(states, ~ coef(lm(positive ~ -1 + since1, data = g[g$state == .x, ]))))
g$slope <- unname(betas[match(g$state, sub("state", "", states))])
...
g2 <- g %>%
  ...
  mutate(max = max(positive),
    slope = slope / max) %>%
  ungroup()
slope_mean <- mean(g2$slope[!duplicated(g2$state)])
p2 <- g2 %>%
  filter(since1>=0) %>%
  arrange(desc(max)) %>%
  filter(state %in% unique(state)[1:49]) %>%
  ...
  scale_y_continuous(labels = function(x) sub(",000,000", "m", sub("\\,000", "k", prettyNum(x, big.mark = ","))),
    expand = expand_scale(0.075)) +
  ...
  theme(plot.background = element_rect(fill = "#f3f3f3", color = "transparent"),
    ...
    panel.grid.minor = element_line(size = rel(0.07), color = "#777777aa"),
    panel.background = element_rect(fill = "#f3f3f3", color = "transparent"),
    strip.text = element_text(family = font, size = rel(1.0),
      margin = margin(0.1, -1, 0.1, -1, "lines")),
    plot.subtitle = element_text(family = font,size = rel(1.325),
      margin = margin(0.1, -1, 0.1, -1, "lines"), face = "plain"),
    plot.title = element_text(family = font,size = rel(2.6),
      margin = margin(0.1, -1, 0.1, -1, "lines"), face = "bold"),
    axis.title = element_text(family = font,face = "italic",color = "#555555",
      size = rel(1.1), hjust = 0.95),
    strip.text.x = element_text(family = font),
    plot.caption = element_text(family = "Font Awesome 5 Brands",
      margin = margin(1.5, 0, 0.1, 0, "lines"), color = "#888888"),
    plot.tag = element_text(family = "Font Awesome 5 Brands", size = rel(0.4), vjust = 0,
      margin = margin(1.5, 0, 0.1, 0, "lines"), color = "#888888"),
    axis.text = element_text(family = font, size = rel(0.7)),
    legend.position = "none") +
  labs(x = "Days Since First Case",
    y = "Reported Cases",
    subtitle = "Daily number of reported cases for each state according to data from covidtracking.com",
    caption = dataviz::theme_mwk_caption_text(),
    title = "COVID-19 Reported Number of Cases by State") +
  ggsave("man/figures/README-state-cases.png",
    width = 8.3, height = 8, units = "in", dpi = 312)
